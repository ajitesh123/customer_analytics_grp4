...
## Question 1: Step 3
```{r}
avg_order_db <- data.nptb.train %>% filter(buyer==1)%>%
  group_by(message)%>%
  summarise(mean(total_os))
avg_order_db
```
...
```{r}
data.nptb.test <- data.nptb.test %>%
mutate(epr.backcountry = pr.backcountry *64.03409*0.40, epr.control = pr.control*49.90060*0.40, epr.endurance = pr.endurance*55.58489*0.40, epr.racquet=pr.racquet*56.40562*0.40, epr.strength = pr.strength*56.70875*0.40, epr.team = pr.team*56.52245*0.40, epr.water = pr.water*61.95734*0.40)
head(data.nptb.test)
```
